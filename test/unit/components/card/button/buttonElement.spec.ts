...
describe("the Aurelia Materialize CSS ButtonElement", () => {
    let sut;

    beforeEach(() =>
        sut = StageComponent
                .withResources("./../../../../base/dist/amd/components/button/buttonElement")
                .inView("<m:button></m:button>"));
    ...
    it("must add the class 'btn-flat' to the given element if flat is set to true on attached", done =>
        // arrange
        sut.inView("<m:button flat='true'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
    ...
    it("must add the class 'btn-floating' to the given element if floating is set to true on attached", done =>
        // arrange
        sut.inView("<m:button floating='true'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
    ...
    it("must add the class 'btn-large' to the given element if large is set to true on attached", done =>
        // arrange
        sut.inView("<m:button large='true'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
    ...
    it("must add the class 'waves-light' to the given element if waves is set to 'light' on attached", done =>
        // arrange
        sut.inView("<m:button waves='light'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
    ...
    it("must not add wave colour if waves is set to 'foo' (a not allowed colour) on attached", done =>
        // arrange
        sut.inView("<m:button waves='foo'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
   ...
   it("must add the class 'disabled' to the given element if disabled is set to true on attached", done =>
        // arrange
        sut.inView("<m:button disabled='true'></m:button>")
            .manuallyHandleLifecycle()
            .create(bootstrap)
            .then(() => sut.bind())
            .then(() => sut.viewModel.element = sut.element)
